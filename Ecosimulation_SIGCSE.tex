...
\linespread{0.965} % use to squeeze lines together, if necessary
...
We also present examples of student-written code that demonstrates their understanding of some basic
...
\STATE{when bored}
  \INDSTATE{for each pacman}
    \INDSTATE[2]{do in any order}
      \INDSTATE[3]{move the pacman's position a random number\\\hspace*{5\algorithmicindent} between 10 and 50 right}
      \INDSTATE[3]{move the pacman's position a random number\\\hspace*{5\algorithmicindent} between 10 and 50 left}
      \INDSTATE[3]{move the pacman's position a random number\\\hspace*{5\algorithmicindent} between 10 and 50 up}
      \INDSTATE[3]{move the pacman's position a random number\\\hspace*{5\algorithmicindent} between 10 and 50 down}
\STATE
...
% However, until recently advances in individual processor speed meant single-processor machines were sufficient for most tasks, and the bulk of programming practices and pedagogy were developed for a single-processor programming model.
...
This work was supported in part by NSF grants BPC-0739259, BPC-104282, CNS-0747273, CSR-0916908, and CNS-0964627; and a grant from Microsoft. We would like to thank 
