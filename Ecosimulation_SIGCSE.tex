...
\linespread{0.98} % use to squeeze lines together, if necessary
...
\and  % use '\and' if you need 'another row' of author names
       ...
       \affaddr{<chg5w,lat7h,cohoon,hazelwood>@virginia.edu}
}
...
An alternate approach to teaching parallel concepts is to utilize robotic platforms,
such as Mindstorms~\cite{Mindstorms}, Pico\-Cricket~\cite{Pico}, Finch~\cite{Finch}, Arduino~\cite{Arduino}, etc.
When using multiple robots (or robots with multiple processors) elements of parallelism are unavoidable and concrete.
However, we are unaware of any effort to utilize these platforms for teaching parallel programming concepts.
It is also not clear that the kinds of parallel challenges that are most apparent in robotics are directly transferable to other forms of parallel programming.
...
~ %%%%%%%%%%% For orphan control; feel free to remove
...
The least constrained is the \code{do in any order} block:
\begin{snippet}
\STATE for each position
\INDSTATE do in any order
\INDSTATE[2] replace the position's x with 5
\INDSTATE[2] replace the position's y with 4
\end{snippet}
In an \code{any order} block each action happens once, but in any arbitrary order or even all at once by different processors.
More structured is the \code{do in order} block:
\begin{snippet}
\STATE for each number
\INDSTATE do in order
\INDSTATE[2] replace the number with 3
\INDSTATE[2] replace the number with the old number + 1
\end{snippet}
Ordered blocks require that the first statement be completed before beginning the second statement.
However, preceding code does not necessarily result in all numbers being 4: another processor could change the values of the numbers between the two in-order statements.
To prevent such interruption we added \code{do in order atomically} blocks that implicitly grab reader/writer locks on everything they touch.
