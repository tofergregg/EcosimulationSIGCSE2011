...
there is a greater need for programmers who are proficient in designing parallel programs, 
...
parallel programming before a second or third programming course.  Traditionally, many colleges and
universities in the United States provided only a single parallel programming course,
often as a senior-level undergraduate elective. 
This means that programmers might not receive any traditional instruction in parallel programming at all.
...
Python~\cite{gpgpuLanguages}.  The trend towards increasing the number cores on each 
machine does not show any signs of abating in the near future~\cite{multicoreTrends}.
...
\EcoSim{} uses an English-like syntax that requires little explanation 
...
and which gave the students step-by-step feedback on the behavior of each emulated processor.
...
MPI, and CUDA).
These studies demonstrated that concrete examples of parallel
code and significant classroom programming time motivated students more than abstract concepts
delivered in a lecture.
\FIXME{Not clear if these studies were with previously-sequential programmers or with novices}
...
While there are many more graphical programming languages than we can enumerate here,
...
\begin{description}
	...
	\item[Engaging:]
		...
		In particular, we wanted to be avoid ``Hello World'' and other text processing.
\end{description}
...
We decided to structure the \EcoSim{} environment with three basic interfaces:
the code entry pane, 
...
This automated display was selected instead of providing some more abstract representation of objects
or an explicit graphics API
in order to minimize the time we spent discussing the display.
...
In practice we found anonymity was also valuable because it removing the need 
...
Multiple fields (such as ``color'' above) can only be accessed by compile-time ordinals like ``5th'';
